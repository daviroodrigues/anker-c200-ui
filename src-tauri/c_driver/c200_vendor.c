...
int c200_vendor_get_u16(int fd, uint8_t selector, uint16_t *value, uint16_t value_offset)
{
	...
	if (value == NULL || value_offset + 1 >= C200_VENDOR_MAX_PAYLOAD_SIZE) {
		errno = EINVAL;
		return -1;
	}
...
int c200_vendor_get_mic(int fd, uint8_t *mode)
{
	uint8_t payload[C200_VENDOR_MAX_PAYLOAD_SIZE];
	uint16_t size = 0;
	...
	if (c200_vendor_get_payload(fd, 0x10, payload, &size) < 0) {
		return -1;
	}
...
int c200_vendor_set_mic(int fd, uint8_t mode)
{
	uint8_t payload[C200_VENDOR_MAX_PAYLOAD_SIZE];
	uint16_t size = 0;
	...
	payload[1] = 0x01;
	payload[5] = mode;
	return c200_vendor_set_payload(fd, 0x10, payload, size);
}
